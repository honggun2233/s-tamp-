...
# ── 색상 상수 ────────────────────────────────────────────
BLUE_DARK  = RGBColor(0x00, 0x41, 0x8A)   # 삼성 블루
...
def heading(text, level=1):
    ...
    if level == 1:
        add_run(para, text, bold=True, size=14, color=BLUE_DARK)
    elif level == 2:
        add_run(para, text, bold=True, size=11, color=BLUE_LIGHT)
    else:
        add_run(para, text, bold=True, size=10, color=BLACK)
    return para

def body(text, size=10, indent=0, color=None, space_after=4):
    para = doc.add_paragraph()
    para.paragraph_format.left_indent = Cm(indent)
    para.paragraph_format.space_after = Pt(space_after)
    add_run(para, text, size=size, color=color or GRAY_TEXT)
    return para

def scale_table(question_num, question_text, low_label, high_label, required=True):
    """5점 척도 테이블 생성"""
    req = ' *' if required else ''
    p = doc.add_paragraph()
    p.paragraph_format.space_before = Pt(8)
    p.paragraph_format.space_after  = Pt(2)
    add_run(p, f'Q{question_num}.', bold=True, size=10, color=BLUE_DARK)
    add_run(p, f' {question_text}{req}', bold=False, size=10, color=BLACK)

    tbl = doc.add_table(rows=3, cols=7)
    tbl.style = 'Table Grid'
    tbl.autofit = False

    col_widths = [Cm(4.5), Cm(1.8), Cm(1.8), Cm(1.8), Cm(1.8), Cm(1.8), Cm(4.5)]
...
def choice_question(question_num, question_text, options, multi=False, required=True, note=''):
    req = ' *' if required else ''
    p = doc.add_paragraph()
    p.paragraph_format.space_before = Pt(8)
    p.paragraph_format.space_after  = Pt(2)
...
def open_question(question_num, question_text, lines=4, required=False):
    req = ' *' if required else ''
    p = doc.add_paragraph()
    p.paragraph_format.space_before = Pt(8)
    p.paragraph_format.space_after  = Pt(2)
...
#  섹션 1 — 응답자 정보
# ════════════════════════════════════════════════════════
section_divider('섹션 1.  응답자 기본 정보')

choice_question(1, '소속 부서를 선택해 주세요.', [
    '운용본부', '리스크관리팀', '컴플라이언스팀', '경영지원팀',
    'IT/디지털혁신팀', '영업본부', '기타: ____________'
])

choice_question(2, '직급을 선택해 주세요.', [
    '사원/주임', '대리', '과장', '차장', '부장', '임원'
])

choice_question(3, 'S-TAMP 시스템 사용 빈도는 어느 정도입니까?', [
    '하루 5회 이상', '하루 1~4회', '주 2~4회', '주 1회 이하'
])

# ════════════════════════════════════════════════════════
#  섹션 2 — 접근성 및 편의성
# ════════════════════════════════════════════════════════
section_divider('섹션 2.  시스템 접근성 및 사용 편의성')

scale_table(4,
    '로그인 및 본인 인증(공동인증서/간편인증) 과정이 편리하였습니까?',
    '매우 불편', '매우 편리')
...
choice_question(10,
    ...
    ['중복 입력 해소', '결재 대기 시간 단축', '시스템 간 이동 불필요',
     '휴먼 에러 감소', '기타: ____________'],
    multi=True,
    note='(복수 선택 가능)')
...
choice_question(15,
    '모바일(스마트폰/태블릿)에서 서명 승인 기능을 사용하셨습니까?', [
        '예  →  Q16에 응답해 주세요',
        '아니오  →  Q17로 이동'
    ])

scale_table(16,
    '(모바일 사용 시) 모바일 환경에서의 사용 편의성은 어떠하였습니까?',
    '매우 불편', '매우 편리')
...
choice_question(23,
    '향후 모바일 생체인증(지문/안면인식) 기능 추가 시 활용 의향이 있으십니까?', [
        '적극 활용하겠다',
        '활용 의향이 있다',
        '모르겠다',
        '활용 의향이 없다'
...
choice_question(26,
    ...
    ['Knox-ATIS 이중결재 통합', '3단계 간소화 프로세스',
     '블록체인 Audit Trail', '당일 즉시 처리', '모바일 승인',
     '중앙 문서 보관·조회', '기타: ____________'],
    multi=True, note='(복수 선택 가능)')

choice_question(27,
    '가장 시급하게 개선이 필요한 부분은 무엇입니까?',
    ['로그인/인증 절차', '화면 UI 구성', '처리 속도', '오류 안내 메시지',
     '모바일 기능', '문서 검색·조회', '교육/도움말', '기타: ____________'],
    multi=True, note='(복수 선택 가능)')
...
add_run(footer_p, '삼성자산운용 디지털혁신팀  |  문의: digital@samsung.com  |  2026. 04',
